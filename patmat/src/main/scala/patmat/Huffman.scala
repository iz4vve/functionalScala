...
object Huffman {
   ...
   *   }
   */
    def times(chars: List[Char]): List[(Char, Int)] = chars.groupBy(c => c).map(c => (c._1, c._2.size)).toList
   ...
   */
    ...
    freq.sortWith(_._2 < _._2) map { case (char, weight) => Leaf(char, weight) }
  }
   ...
   */
  def until(
             condition: List[CodeTree] => Boolean, action: List[CodeTree] => List[CodeTree]
   ...
   */
    def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
      def decodeAcc(pTree: CodeTree, remaining: List[Bit], acc: List[Char]): List[Char] = {

        (pTree, remaining) match {
   ...
   */
    def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      def encodeChar(pTree: CodeTree, char: Char, acc: List[Bit]): List[Bit] = {
        pTree match {
          case pTree: Leaf => acc
   ...
   */
    def convert(tree: CodeTree): CodeTable = {
      ...
      def convertAcc(pTree: CodeTree, acc: CodeTable): CodeTable = {
        pTree match {
          case t: Fork => mergeCodeTables(
            convertAcc(t.left, add(chars(t.left), 0, acc)), convertAcc(t.right, add(chars(t.right), 1, acc))
          )
          case t: Leaf => acc
        }
      }
      convertAcc(tree, List[(Char, List[Bit])]())
    }
